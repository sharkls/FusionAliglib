...
void nms(std::vector<yolov5Detection>& res, float *output, std::vector<float> conf_thresh, float nms_thresh = 0.5);

void batch_nms(std::vector<std::vector<yolov5Detection>>& batch_res, float *output, int batch_size, int output_size, std::vector<float> conf_thresh, float nms_thresh = 0.5);
