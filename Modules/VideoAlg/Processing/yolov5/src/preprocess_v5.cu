...
/*
* CUDA核函数：warpaffine_kernel
* 功能：执行仿射变换并将图像数据从BGR格式转换为RGB格式，同时进行归一化
* 参数:
*   src: 源图像数据指针
*   src_line_size: 源图像每行的字节数
*   src_width: 源图像宽度
*   src_height: 源图像高度
*   dst: 目标图像数据指针
*   dst_width: 目标图像宽度
*   dst_height: 目标图像高度
*   const_value_st: 填充常量值
*   d2s: 源到目标的仿射变换矩阵
*   edge: 处理边界的像素数量
*/
...
// /*
// * 函数：cuda_preprocess
// * 功能：执行图像预处理，包括内存拷贝和调用warpaffine_kernel核函数
// * 参数:
// *   src: 源图像数据指针
// *   src_width: 源图像宽度
// *   src_height: 源图像高度
...
__global__ void warpaffine_kernel(
    ...
    int src_height, float* dst, int dst_width,
    int dst_height, uint8_t const_value_st,
    AffineMatrix d2s, int edge, bool skip_affine) {
  int position = blockDim.x * blockIdx.x + threadIdx.x;
  if (position >= edge) return;
  ...
  if (!skip_affine) {
    ...
    int dx = position % dst_width;
    int dy = position / dst_width;
    src_x = m_x1 * dx + m_y1 * dy + m_z1 + 0.5f;
    src_y = m_x2 * dx + m_y2 * dy + m_z2 + 0.5f;
  } else {
    src_x = position % src_width + 0.5f;
    src_y = position / src_width + 0.5f;
  }

  float c0, c1, c2;
  if (!skip_affine && (src_x <= -1 || src_x >= src_width || src_y <= -1 || src_y >= src_height)) {
    // out of range
    c0 = const_value_st;
    c1 = const_value_st;
    c2 = const_value_st;
...
void cuda_preprocess(
  ...
  bool skip_affine = src_width == dst_width && src_height == dst_height;      // 判断目标尺寸和源尺寸是否一致
  int img_size = src_width * src_height * 3;
  // copy data to pinned memory
  memcpy(img_buffer_host, src, img_size);
  // copy data to device memory
  ...
  warpaffine_kernel<<<blocks, threads, 0, stream>>>(
      img_buffer_device, src_width * 3, src_width,
      src_height, dst, dst_width,
      dst_height, 128, d2s, jobs, skip_affine);
}

/*
* 函数：cuda_batch_preprocess
* 功能：对一批图像进行预处理
* 参数:
*   img_batch: 图像批次的向量
*   dst: 目标图像数据指针
