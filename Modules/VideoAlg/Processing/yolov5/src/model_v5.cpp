...
*   file : string 权重文件路径
* 返回：
*   std::map<std::string, Weights> : 权重映射表
*/
...
/*
* 功能：计算调整后的宽度，以确保宽度是divisor的倍数
* 参数：
*   x : int 原始宽度
*   gw : float 宽度调整因子
*   divisor : int 宽度需要是该数的倍数，默认为8
* 返回：
*   int : 调整后的宽度
*/
...
* 功能：添加批量归一化层
* 参数：
*   network : INetworkDefinition* 网络定义
*   weightMap : std::map<std::string, Weights>& 权重映射表
*   input : ITensor& 输入张量
*   lname : string 层名称
*   eps : float 用于避免除以零的小值
* 返回：
*   IScaleLayer* : 批量归一化层
*/
...
/*
* 功能：添加Focus层，用于切片和重排操作
* 参数：
*   network : INetworkDefinition* 网络定义指针
*   weightMap : std::map<std::string, Weights>& 权重映射表
*   input : ITensor& 输入张量
*   inch : int 输入通道数
*   outch : int 输出通道数
*   ksize : int 卷积核大小
*   lname : std::string 层名称
* 返回：
*   ILayer* : Focus层的输出层指针
*/
...
/*
* 功能：添加C3层，YOLOv5的特色结构，结合了CSP瓶颈层和残差连接
* 参数：
*   network : INetworkDefinition* 网络定义指针
*   weightMap : std::map<std::string, Weights>& 权重映射表
*   input : ITensor& 输入张量
...
/*
* 功能：添加SPPF层，是SPP层的快速版本，使用连续的最大池化层
* 参数：
*   network : INetworkDefinition* 网络定义指针
*   weightMap : std::map<std::string, Weights>& 权重映射表
*   input : ITensor& 输入张量
...
* 参数：
*   weightMap : std::map<std::string, Weights>& 权重映射表
*   lname : std::string 层名称前缀
* 返回：
*   std::vector<std::vector<float>> : 锚点的二维向量
*/
...
* 功能：构建并返回TensorRT引擎
* 参数：
*   maxBatchSize : unsigned int 最大批量大小
*   builder : IBuilder* 网络构建器
*   config : IBuilderConfig* 网络配置
*   dt : DataType 数据类型
*   gd : float& 深度增益
*   gw : float& 宽度增益
*   wts_name : string& 权重文件名
* 返回：
*   ICudaEngine* : 构建的TensorRT引擎
*/
...
* 功能：构建用于目标检测的P6引擎
* 参数：
*   maxBatchSize : unsigned int 最大批处理大小
*   builder : IBuilder* 构建器指针
*   config : IBuilderConfig* 构建器配置指针
*   dt : DataType 数据类型
*   gd : float& 深度增益
*   gw : float& 宽度增益
*   wts_name : std::string& 权重文件名
* 返回：
*   ICudaEngine* : 构建好的CUDA引擎指针
*/
