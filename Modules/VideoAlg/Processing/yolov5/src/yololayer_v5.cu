...
Dims YoloLayerPlugin::getOutputDimensions(int index, const Dims* inputs, int nbInputDims) TRT_NOEXCEPT {
  ...
  int totalsize = mMaxOutObject * sizeof(yolov5Detection) / sizeof(float);
...
__global__ void CalDetection(const float *input, float *output, int noElements,
  ...
  for (int k = 0; k < kNumAnchor; ++k) {
    ...
    char *data = (char*)res_count + sizeof(float) + count * sizeof(yolov5Detection);
    yolov5Detection *det = (yolov5Detection*)(data);
...
void YoloLayerPlugin::forwardGpu(const float* const* inputs, float *output, cudaStream_t stream, int batchSize) {
  int outputElem = 1 + mMaxOutObject * sizeof(yolov5Detection) / sizeof(float);
